...
lg_cap = lgs[lgs.LG_ID.isin([str(x) for x in selected_lgs])]["Storage_Capacity_tons"].sum()
...
KP=[("Total CG→LG (t)",f"{cg_sel:,.1f}"),("Total LG→FPS (t)",f"{lg_sel:,.1f}"),
    ("Avg CG→LG (t/d)",f"{avg_cg:,.1f}"),("Avg LG→FPS (t/d)",f"{avg_lg:,.1f}"),
    ...
    ("FPS At-Risk",f"{fps_r}"),("% Plan Completed",f"{pct_p:.1f}%"),
