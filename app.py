...
# 2. Helper - export DataFrame to Excel bytes
def to_excel(df):
    buf = BytesIO()
...
def load_data(fn):
    ...
    # Ensure LG_ID columns are strings for merging
...
# Pre-dispatch offset X for negative days
...
for d in range(1, DAYS + 1):
    need = dt.get(d, 0)
    cum_req += need
    over = (cum_req - DAILY_CAP * d) / DAILY_CAP
...
fps_stock = stock_levels[stock_levels.Entity_Type=="FPS"].merge(fps[["FPS_ID","Reorder_Threshold_tons","LG_ID"]], left_on="Entity_ID", right_on="FPS_ID")
...
arf = fps_stock.query("Day>=1 & Day<=@day_range[1] & At_Risk & LG_ID in @selected_lgs")
...
st.download_button("Download Excel", to_excel(report), f"FPS_Report_{day_range[0]}_{day_range[1]}.xlsx",
                   "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
pdf_buf = BytesIO()
