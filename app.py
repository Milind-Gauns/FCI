...
# 2. Helper: DataFrame → Excel bytes
def to_excel(df):
    buf = BytesIO()
    df.to_excel(buf, index=False)
    return buf.getvalue()

# 3. Load & cache all sheets
@st.cache_data
def load_data(fn):
...
MAX_TRIPS = int(settings.loc[settings.Parameter=="Vehicles_Total","Value"].iloc[0]) * int(settings.loc[settings.Parameter=="Max_Trips_Per_Vehicle_Per_Day","Value"].iloc[0])
...
# Detect correct entity-type column
...
with tab5:
    ...
    for fid in report["FPS_ID"]:
        ...
        future = dispatch_lg[(dispatch_lg.FPS_ID==fid)&(dispatch_lg.Day> end_day)]["Day"]
        nd = int(future.min()) if not future.empty else None
        days_to = nd - end_day if nd else None
        fps_data.append({"FPS_ID":fid,"Current_Stock_tons":stock_now,"Next_Receipt_Day":nd,"Days_To_Receipt":days_to})
...
with tab6:
    ...
    st.download_button("PDF", pdf_buf.getvalue(), "FPS_Report.pdf", "application/pdf")
...
with tab7:
    ...
    avg_tr   = veh_usage.query("Day>=1 & Day<=@day_range[1]")["Trucks_Used"].mean()
    ...
    metrics = [
        ...
        ("Avg Daily CG→LG (t/d)", f"{avg_cg:,.1f}"),
        ("Avg Daily LG→FPS (t/d)",f"{avg_lg:,.1f}"),
        ("Avg Trips/Day",         f"{avg_tr:.1f}"),
        ("% Fleet Utilization",   f"{pct_f:.1f}%"),
        ("LG Stock on Hand (t)",  f"{lg_on:,.1f}"),
        ("FPS Stock on Hand (t)", f"{fps_on:,.1f}"),
        ("% LG Capacity Filled",  f"{pct_lg:.1f}%"),
